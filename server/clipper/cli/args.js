...
const args = require('yargs')
  ...
  .command('$0 [url] [output]', 'Save a page into a single HTML file.', (yargs) => {
    yargs.positional('url', {
      description: 'URL or path on the filesystem of the page to save',
      type: 'string',
    });
  ...
  .default({
    ...
    'save-raw-page': false,
    'web-driver-executable-path': '/Users/wang.boyang/Downloads/chromedriver',
  ...
  .options('browser-headless', {
    description:
      'Run the browser in headless mode (puppeteer, webdriver-gecko, webdriver-chromium)',
  })
  .boolean('browser-headless')
  .options('browser-executable-path', {
    description:
      'Path to chrome/chromium executable (puppeteer, webdriver-gecko, webdriver-chromium)',
  })
  ...
  .options('browser-load-max-time', {
    description:
      'Maximum delay of time to wait for page loading in ms (puppeteer, webdriver-gecko, webdriver-chromium)',
  })
  .number('browser-load-max-time')
  .options('browser-wait-until', {
    description:
      'When to consider the page is loaded (puppeteer, webdriver-gecko, webdriver-chromium)',
  })
  .choices('browser-wait-until', ['networkidle0', 'networkidle2', 'load', 'domcontentloaded'])
  .options('browser-wait-until-fallback', {
    description: 'Retry with the next value of --browser-wait-until when a timeout error is thrown',
  })
  .boolean('browser-wait-until-fallback')
  .options('browser-debug', {
    description: 'Enable debug mode (puppeteer, webdriver-gecko, webdriver-chromium)',
  })
  .boolean('browser-debug')
  .options('browser-extensions', {
    description:
      "List of extension paths separated by a space and relative to the 'cli' folder (webdriver-gecko, webdriver-chromium)",
  })
  .array('browser-extensions')
  .options('browser-scripts', {
    description:
      "List of script paths separated by a space and relative to the 'cli' folder. They will be executed in all the frames.",
  })
  .array('browser-scripts')
  .options('browser-args', {
    description:
      'Arguments provided as a JSON array and passed to the browser (puppeteer, webdriver-gecko, webdriver-chromium)',
  })
  .string('browser-args')
  ...
  .boolean('crawl-links')
  .options('crawl-inner-links-only', {
    description: 'Crawl pages found via inner links only if they are hosted on the same domain',
  })
  .boolean('crawl-inner-links-only')
  .options('crawl-load-session', {
    description:
      'Name of the file of the session to load (previously saved with --crawl-save-session or --crawl-sync-session)',
  })
  ...
  .options('crawl-save-session', {
    description: 'Name of the file where to save the state of the session',
  })
  .string('crawl-save-session')
  .options('crawl-sync-session', {
    description: 'Name of the file where to load and save the state of the session',
  })
  .string('crawl-sync-session')
  .options('crawl-max-depth', {
    description: 'Max depth when crawling pages found in internal and external links (0: infinite)',
  })
  .number('crawl-max-depth')
  .options('crawl-external-links-max-depth', {
    description: 'Max depth when crawling pages found in external links (0: infinite)',
  })
  .number('crawl-external-links-max-depth')
  .options('crawl-replace-urls', {
    description: 'Replace URLs of saved pages with relative paths of saved pages on the filesystem',
  })
  .boolean('crawl-replace-urls')
  .options('crawl-rewrite-rules', {
    description: 'List of rewrite rules used to rewrite URLs of internal and external links',
  })
  ...
  .options('filename-replacement-character', {
    description: 'The character used for replacing invalid characters in filenames',
  })
  .string('filename-replacement-character')
  .string('filename-replacement-character')
  .options('group-duplicate-images', {
    description: 'Group duplicate images into CSS custom properties',
  })
  ...
  .options('load-deferred-images', {
    description:
      'Load deferred (a.k.a. lazy-loaded) images (puppeteer, webdriver-gecko, webdriver-chromium)',
  })
  .boolean('load-deferred-images')
  .options('load-deferred-images-max-idle-time', {
    description:
      'Maximum delay of time to wait for deferred images in ms (puppeteer, webdriver-gecko, webdriver-chromium)',
  })
  .number('load-deferred-images-max-idle-time')
  .options('load-deferred-images-keep-zoom-level', {
    description: 'Load defrrred images by keeping zoomed out the page',
  })
  .boolean('load-deferred-images-keep-zoom-level')
  .options('max-parallel-workers', {
    description:
      'Maximum number of browsers launched in parallel when processing a list of URLs (cf --urls-file)',
  })
  .number('max-parallel-workers')
  .options('max-resource-size-enabled', {
    description: 'Enable removal of embedded resources exceeding a given size',
  })
  .boolean('max-resource-size-enabled')
  .options('max-resource-size', {
    description:
      'Maximum size of embedded resources in MB (i.e. images, stylesheets, scripts and iframes)',
  })
  .number('max-resource-size')
  .options('remove-frames', {
    description: 'Remove frames (puppeteer, webdriver-gecko, webdriver-chromium)',
  })
  .boolean('remove-frames')
  .options('remove-hidden-elements', {
    description: 'Remove HTML elements which are not displayed',
  })
  .boolean('remove-hidden-elements')
  .options('remove-unused-styles', {
    description: 'Remove unused CSS rules and unneeded declarations',
  })
  ...
  .options('remove-alternative-fonts', {
    description: 'Remove alternative fonts to the ones displayed',
  })
  ...
  .options('remove-alternative-images', {
    description: 'Remove images for alternative sizes of screen',
  })
  .boolean('remove-alternative-images')
  .options('save-raw-page', {
    description:
      'Save the original page without interpreting it into the browser (puppeteer, webdriver-gecko, webdriver-chromium)',
  })
  .boolean('save-raw-page')
  .options('urls-file', {
    description: 'Path to a text file containing a list of URLs (separated by a newline) to save',
  })
  .string('urls-file')
  .options('user-agent', {
  ...
  .options('web-driver-executable-path', {
    description: 'Path to Selenium WebDriver executable (webdriver-gecko, webdriver-chromium)',
  })
  .string('web-driver-executable-path').argv;
...
Object.keys(args)
  .filter((optionName) => optionName.includes('-'))
